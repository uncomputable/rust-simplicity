...
/// A witness expression wrapped in `Ok` if satisfaction was successful
/// or wrapped in `Err` if satisfaction failed.
///
/// This information is necessary for the policy satisfier to select the correct
/// disjunction (`or`) and threshold (`thres`) branches.
type SatResult = Result<Arc<WitnessNode<Elements>>, Arc<WitnessNode<Elements>>>;

fn ok_if<T>(bit: bool, value: T) -> Result<T, T> {
    match bit {
        true => Ok(value),
        false => Err(value),
    }
...
fn symmetric_unwrap<T>(result: Result<T, T>) -> T {
    match result {
...
impl<Pk: ToXOnlyPubkey> Policy<Pk> {
    fn satisfy_internal<S: Satisfier<Pk>>(
        ...
        let node = match *self {
            Policy::Unsatisfiable(entropy) => {
                Err(super::serialize::unsatisfiable(inference_context, entropy))
            }
            Policy::Trivial => Ok(super::serialize::trivial(inference_context)),
            Policy::Key(ref key) => {
            ...
            Policy::After(n) => {
                ...
                ok_if(
                    ...
                    super::serialize::after::<Arc<_>>(inference_context, n),
                )
            }
            Policy::Older(n) => ok_if(
                satisfier.check_older(elements::Sequence(n.into())),
                super::serialize::older::<Arc<_>>(inference_context, n),
            ),
            ...
            Policy::And {
                ...
                let left_result = left.satisfy_internal(inference_context, satisfier)?;
                let right_result = right.satisfy_internal(inference_context, satisfier)?;
                let both_ok = left_result.is_ok() && right_result.is_ok();
                let left = symmetric_unwrap(left_result);
                let right = symmetric_unwrap(right_result);
                ok_if(both_ok, super::serialize::and(&left, &right))
            }
            Policy::Or {
                ref left,
                ref right,
                ...
                ok_if(left_ok || right_ok, serialization)
            }
            Policy::Threshold(k, ref subs) => {
                ...
                let nodes: Vec<_> = node_results.into_iter().map(symmetric_unwrap).collect();
                ok_if(
                    all_selected_ok,
            ...
            Policy::Assembly(cmr) => satisfier
                ...
                .map(Ok)
                .ok_or(SatisfierError::MissingAssembly(cmr))?,
        };
        Ok(node)
    }
    ...
    pub fn satisfy<S: Satisfier<Pk>>(
        ...
        let witness_program = match self.satisfy_internal(&types::Context::new(), satisfier)? {
            Ok(result) => result,
            Err(..) => return Err(SatisfierError::Unsatisfiable),
        };
