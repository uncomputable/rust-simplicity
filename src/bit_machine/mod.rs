...
impl BitMachine {
    ...
    pub fn exec<J: Jet>(
        ...
        self.exec_with_tracker(program, env, &mut ())
    }
    ...
    pub(crate) fn exec_prune<J: Jet>(
        ...
        program: &RedeemNode<J>,
        env: &J::Environment,
    ) -> Result<[HashSet<Imr>; 2], ExecutionError> {
        let mut tracker = [HashSet::new(), HashSet::new()];
...
/// A struct that keeps track of which case branches were executed
/// during the execution of the Bit Machine.
///
/// The trait is implemented for `[HashSet<Imr>; 2]`, which does the actual tracking,
/// and it is implemented for `()`, which is a dummy tracker that is optimized out by the compiler.
///
...
impl CaseTracker for [HashSet<Imr>; 2] {
    fn track_left(&mut self, imr: Imr) {
        self[0].insert(imr);
    }
